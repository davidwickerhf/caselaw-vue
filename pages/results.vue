...
import { Loader2, AlertCircle, Filter, Brackets, ArrowLeft } from 'lucide-vue-next'
import AppHeader from '~/components/shared/AppHeader.vue'
import QueryBuilder from '~/components/search/QueryBuilder.vue'
import SmartSearchChip from '~/components/search/SmartSearchChip.vue'
...
import { queryBuilderGroupToSearchQuery, searchQueryToQueryBuilderGroup, searchQueryToTokens } from '~/lib/utils/search-query'
import { paramsToQueryBuilderState, queryBuilderGroupToParams } from '~/lib/utils/query-builder-url'
...
const badgeTokens = computed(() => searchQueryToTokens(store.query.value))

function applyQueryAndSearch(query: SearchQuery, updateUrl: boolean, group?: QueryBuilderGroup) {
  store.setQuery({ ...query, page: query.page || 1 })
  store.search()
  if (updateUrl) {
    const params = queryBuilderGroupToParams(
      group || smartSearch.queryBuilderGroup.value,
      { pageSize: query.pageSize, echrCursor: query.echrCursor, rsCursor: query.rsCursor, searchString: smartSearch.searchString.value || undefined }
    )
    syncingRoute.value = true
    router.replace({ path: '/results', query: Object.fromEntries(params.entries()) }).finally(() => {
...
function applyRouteQuery() {
  ...
  const shouldNormalize = params.has('page') || !params.has('qb')

  if ([...params.keys()].length === 0) {
    if (smartSearch.confirmedTokens.value.length > 0 || smartSearch.freeText.value) {
      const query = smartSearch.buildSearchQuery()
      routeError.value = null
      applyQueryAndSearch({ ...query, page: 1 }, true)
    } else {
      store.resetQuery()
      routeError.value = null
    }
...
watch(
  ...
  () => {
    ...
    routeError.value = null
    applyQueryAndSearch({ ...parsed.query, page: 1 }, true, smartSearch.queryBuilderGroup.value)
  },
  { deep: true }
)
...
function handleSubmit() {
  ...
  routeError.value = null
  applyQueryAndSearch({ ...parsed.query, page: 1 }, true, smartSearch.queryBuilderGroup.value)
  history.add(store.query.value, store.results.value?.total)
}

function handleEditSearch() {
  const params = queryBuilderGroupToParams(
    smartSearch.queryBuilderGroup.value,
    { pageSize: store.query.value.pageSize, searchString: smartSearch.searchString.value || undefined }
  )
  router.push({ path: '/', query: Object.fromEntries(params.entries()) })
}

function handleFilterChange(partial: Partial<SearchQuery>) {
  const next = { ...store.query.value, ...partial, page: 1 }
...
function handleFindSimilar(citation: Citation) {
  const terms = [citation.title, ...(citation.keywords || []).slice(0, 3)].filter(Boolean).join(' ')
...
<template>
  <div class="h-screen overflow-hidden bg-background">
    <AppHeader showSearch fixed />

    <main class="h-full box-border pt-14 overflow-hidden">
      <div class="flex h-full flex-col">
        <!-- Query summary row -->
        <div class="shrink-0 bg-background">
          <div class="flex h-12 w-full items-stretch border-b border-border">
            <button
              class="group flex h-full w-40 items-center justify-center gap-2 px-0 text-sm font-semibold text-muted-foreground transition-all duration-200 ease-out hover:text-foreground hover:bg-muted/40 active:scale-[0.98]"
            ...
            <div class="flex-1 overflow-x-auto">
              <div class="flex h-full items-center gap-2 px-6">
                <template v-if="badgeTokens.length > 0">
                  <SmartSearchChip
                    v-for="token in badgeTokens"
                    :key="token.id"
                    :token="token"
                    :removable="false"
                    size="compact"
                ...
                <span v-else class="text-xs text-muted-foreground">No search parameters</span>
              </div>
            </div>
            ...
            >
              <Brackets class="h-4 w-4 transition-transform group-hover:scale-105" />
              Query Builder
            </button>
          </div>
        ...
        <!-- Main content -->
        <div class="mx-auto flex w-full max-w-screen-2xl min-h-0 flex-1 gap-0 px-0">
          ...
          <!-- Center: Results -->
          <section class="flex min-h-0 flex-1 flex-col pt-6 pr-6 pb-0 pl-6 min-w-0">
            <!-- Loading -->
            <div v-if="store.loading.value" class="flex flex-1 items-center justify-center">
              <div class="text-center space-y-3">
                <Loader2 class="h-8 w-8 animate-spin text-muted-foreground mx-auto" />
                <p class="text-sm text-muted-foreground">Searching cases...</p>
              </div>
            </div>

            <!-- Error -->
            <div v-else-if="store.error.value" class="flex flex-1 items-center justify-center">
              <div class="text-center space-y-3 max-w-md">
                <AlertCircle class="h-8 w-8 text-red-500 mx-auto" />
                <p class="text-sm font-medium text-foreground">Search failed</p>
                <p class="text-xs text-muted-foreground">{{ store.error.value }}</p>
            ...
            <!-- Results -->
            <div v-else-if="store.results.value" class="flex min-h-0 flex-1 flex-col gap-4">
              <!-- Stats -->
              <ResultStats
              ...
              >
                <template #countPrefix>
                  <Button variant="outline" size="sm" class="h-7 gap-1.5" @click="filtersCollapsed = !filtersCollapsed">
              ...
              <div class="flex items-center justify-between gap-4">
                <ResultSort
                  ...
                  :view-mode="viewMode"
                  @change="(sortBy: string, dir: string) => applyQueryAndSearch({ ...store.query.value, sortBy: sortBy as 'relevance' | 'date' | 'citations' | 'importance', sortDirection: dir as 'asc' | 'desc', page: 1 }, true)"
              ...
              <div class="min-h-0 flex-1 overflow-y-auto pr-0 pb-16">
                <ResultList
                  ...
                  @page="(p) => applyQueryAndSearch({ ...store.query.value, page: p }, true)"
                />
              </div>
            </div>
            ...
            <div v-else class="flex flex-1 items-center justify-center">
              <p class="text-muted-foreground">Enter a search query to find cases.</p>
            </div>
          </section>
        </div>
      </div>
